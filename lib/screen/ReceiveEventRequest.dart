...
class ReceiveEventrequest extends StatefulWidget {
  ...
  ReceiveEventrequest(
      {required this.eventReq, required this.gid, required this.usedContent});
...
class _receivedEventRequest extends State<ReceiveEventrequest> {
  ...
  Future<void> getPrimaryCalendar() async {
    primaryCalendar = await GetGroupPrimaryCalendar()
        .getGroupPrimaryCalendar(widget.gid, uid);
  }

  Future<void> _acceptRequest(eventRequest request) async {
    await getPrimaryCalendar();
    print(
        'uid: $uid gid: ${widget.gid} request: ${request.uid} ${request.event_request_id} ${primaryCalendar}');
    ReceiveEventRequest().receiveEventRequest(uid, request.uid, widget.gid,
        request.event_request_id, primaryCalendar);
            ...
            TextButton(
              onPressed: () async {
                RejectEventRequest().rejectEventRequest(
                    uid, request.uid, widget.gid, request.event_request_id);
                _requests.remove(request);
              ...
              child: Text('拒否', style: TextStyle(color: Colors.red)),
            ),
          ],
        );
      },
                ...
                children: [
                  SizedBox(
                    height: MediaQuery.of(context).size.height *
                        0.8, // Provides enough space to allow pull-to-refresh
                    child: Center(
                      child: Row(
