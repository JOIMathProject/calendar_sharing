...
class _ContentsManageState extends State<ContentsManage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    Timer.periodic(Duration(seconds: 2), (timer) {
      setState(() {});
      //print("contents_length: ${widget.contents.length}");
      if (!mounted) {
        timer.cancel();
      }
    });
  ...
  List<GroupInformation> _filteredContents({bool isPersonal = false, bool isGroup = false}) {
    ...
    if (isPersonal) {
      filteredContents = widget.contents.where((content) => content.is_friends == '1').toList();
    } else if (isGroup) {
      filteredContents = widget.contents.where((content) => content.is_friends == '0').toList();
    } else {
